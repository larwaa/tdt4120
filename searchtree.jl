...
function searchtree(root::Node, dna::String)::Int64
    if isempty(dna)
        return root.count
    elseif haskey(root.children, dna[1])
        return searchtree(root.children[dna[1]], dna[2:end])
    end
    return 0
end
...
root1 = Node(
    Dict('A' => Node(Dict{Char,Node}(), 1),'G' => Node(Dict('A' => Node(Dict{Char,Node}(), 2)), 1)), 0)
