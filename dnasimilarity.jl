...
function addchildren(root, dnasequence)
    if dnasequence != ""
    	if haskey(root.children, dnasequence[1])
    		root.children[dnasequence[1]].count += 1
		else
			root.children[dnasequence[1]] = Node(Dict{Char, Node}(), 1)
		end
        if lastindex(dnasequence) >= 2
            addchildren(root.children[dnasequence[1]], dnasequence[2:end])
        end
    end
end
